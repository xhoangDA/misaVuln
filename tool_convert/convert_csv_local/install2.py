...
def extract_refs_urls(data_field: str) -> str:
    """
    Trích xuất dữ liệu từ mảng refs và trả về chuỗi định dạng theo yêu cầu.
    """
    refs_urls = ""
    ...
    try:
        # Chuẩn hóa chuỗi JSON (thay dấu ' thành ")
        normalized = data_field.replace("'", '"')
        
        # Parse thành list các dictionary
        refs_list = json.loads(normalized)
        
        # Lọc ra các URL (bắt đầu bằng http/https)
        urls = [
            ref['name'] for ref in refs_list 
            if isinstance(ref, dict) and 
            'name' in ref and 
            re.match(r'^https?://', ref['name'])
        ]
        refs_urls = ', '.join(urls)
        return refs_urls
...
def process_and_group_data(data):
    ...
    for name, services in grouped.items():
        ...
        merged['desc'] = merged.get('desc', '').replace('\n  \n', '\n').replace(" '- Summary: ", "")
        merged['refs'] = extract_refs_urls(merged['refs'])
        merged['cve'] = re.sub(r"[\[\]']", '', merged['cve'])
...
\t- Affected services/hosts detected:
...
def create_jira_json(issues_data, json_file_path):
    ...
    base_template = {
        ...
        "fields": {
            ...
            "labels": ["Ticket" , "Openvas"],   # Hard code
            "priority": {"id": "3"}  # Hard code
        }
    }
    ...
    for issue_data in issues_data:
        ...
        # new_issue["fields"]["customfield_10119"] = issue_description
        new_issue["fields"]["description"] = issue_description
                
        issue_updates.append(new_issue)
        count += 1
    
    print(count)
    result = {"issueUpdates": issue_updates}
    with open(json_file_path, mode='w', encoding='utf-8') as json_file:
